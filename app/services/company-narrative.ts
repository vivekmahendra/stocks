...
export function generateCompanyNarrativePrompt(
  symbol: string,
  companyName?: string
): string {
...
3. Notable opinions from serious investors and analysts on X (formerly Twitter)
4. Recent news or events impacting the company
5. Technical levels traders are watching
6. Overall bull vs bear case summary

Focus on actionable insights and current market dynamics. Keep it concise (4-5 sentences) but informative. `;
}
...
export class CompanyNarrativeService {
  ...
  private async cacheNarrative(
    ...
    try {
      ...
      const { error } = await supabase.from("company_narrative_cache").insert({
        symbol,
        narrative,
